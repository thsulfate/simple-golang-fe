	...
	for i := 0; i < count; i++ {
		...
		go func() {
			...
			responses = append(responses, BackendResponse{
				...
				TotalTime: strconv.FormatFloat(float64(diffTime)/float64(time.Second)*1000, 'f', 4, 64) + " ms",
			})
			if backendResp.Hostname == "backend1" {
				backend1Count++
			} else if backendResp.Hostname == "backend2" {
				backend2Count++
			}
			mu.Unlock()
		}()
